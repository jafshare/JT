    ...
    ctx.program
      ...
      .action(async (options) => {
        if (options.ls || Object.keys(options).length === 0) {
          success(
            templateRegistry.data
              .map((tp, index) => `${index + 1}. ${tp.name}`)
              .join("\r\n")
          );
        } else if (options.add) {
          // 填写模板信息
          ...
          success(
            "模板路径:",
            record.local ? record.localPath : record.remoteSrc
          );
          success("------------------------------");
        }
      });
